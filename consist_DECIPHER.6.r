...
# Usage: 
# consist_DECIPHER.6.r \
# --Kgroups_uniq=~/project/TR02/TR02.18/rds/Kgroups_uniq.rds \
# --DetectRepeat_dir=~/project/TR02/TR02.18/aa_DetectRepeats/ \
# --MSA_dir=~/project/TR02/TR02.18/aa_MSA/ \
# --score=10 \
# --out_tsv=out.tsv \
...
opt <- parse_args(OptionParser(usage = "%prog [options] [--Kgroups_uniq=FILENAME] [--DetectRepeat_dir=DIRNAME] [--MSA_dir=DIRNAME] [--score=FLOAT] [--out_tsv=FILENAME] [--out_mean_cons=FILENAME]", option_list=option_list)) 
...
line <- paste('KgroupID', 'n_seqs', 'n_seqs_r', 'n_pairs', 'n_seqs_inpairs', 'mean_cons', sep='\t')
write(line, file=opt$out_tsv, append=FALSE)

for (Kgroup_h in Kgroups_uniq)
{
	# get the filtered results
	DetectRepeats_filename <- paste(opt$DetectRepeat_dir, Kgroup_h, '.DetectRepeats.rds', sep='')
	DRresult_h <- readRDS(DetectRepeats_filename)
	...
	if (nrow(DRresult_h)!=0) # ignore K group if no repeat detect
	{
		...
		frac_repeat <- c(frac_repeat, n_repeat_loci / (n_repeat_loci+n_not_repeat_loci))
		cat(Kgroup_h, mean(n_repeat_loci / (n_repeat_loci+n_not_repeat_loci)), '\n')
		...
		if ( length(pair_consistency) > 0 )
		{
			...
			line <- paste(Kgroup_h, n_seqs, n_seqs_r, length(pair_consistency), length(unique(seqs_included)), mean_h, sep='\t')
			write(line, file=opt$out_tsv, append=TRUE)
		} else {
			line <- paste(Kgroup_h, n_seqs, n_seqs_r, 0, 0, NA, sep='\t')
			write(line, file=opt$out_tsv, append=TRUE)
		}
	} else {
