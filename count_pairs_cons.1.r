...
# Usage: 
# count_cons_seqs.1.r --DR_filename=~/project/TR02/TR02.26/run1/aa_DetectRepeats/K24316.DetectRepeats.rds --MSA_filename=~/project/TR02/TR02.26/run1/aa_MSA/K24316.MSA.rds --score=15 --outfile=~/project/TR02/TR02.26/run1/aa_count_cons_seqs/K24316.tsv
...
if (nrow(DRresult_h)!=0) # ignore K group if no repeat detect
{
    ...
    out_text <- c(ncol(pairs), consider_n_pairs, has_cons_TR)
} else {
    # MSA
    mt_MSA <- get_mt_MSA(MSA_h, MSA_len, n_seqs)
    pairs <- combn(seq(n_seqs), 2)
