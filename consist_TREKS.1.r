...
opt <- parse_args(OptionParser(usage = "%prog [options] [--Kgroups_uniq=FILENAME] [--TREKS_dir=DIRNAME] [--MSA_dir=DIRNAME] [--score=FLOAT] [--out_tsv=FILENAME] [--out_mean_cons=FILENAME]", option_list=option_list)) 
...
get_mt_rep_loci <- function(TREKS_result_h, MSA_h, mt_MSA, MSA_len, n_seqs) {
    ...
    {
        index_h <- which(startWith(names(MSA_h), TREKS_result_h[row_h, 'seqID']))
