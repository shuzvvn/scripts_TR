...
option_list = list(
	...
	make_option(c("-r", "--DR_dir"), type="character", default="NA", help="Directory path for Kgroup.xls (Required)", metavar="dirname"),
...
opt <- parse_args(OptionParser(usage = "%prog [options] [--Kgroups_uniq=FILENAME] [--DR_dir=DIRNAME] [--MSA_dir=DIRNAME] [--score=FLOAT] [--out_tsv=FILENAME]", option_list=option_list)) 
...
get_mt_rep_loci <- function(DR_result_h, MSA_h, mt_MSA, MSA_len, n_seqs) {
	mt_rep_loci <- matrix(0, nrow = MSA_len, ncol = n_seqs)
	for (row_h in seq(nrow(DR_result_h)))
	{
		index_h <- which(startsWith(names(MSA_h), DR_result_h[row_h, 'seqID']))
		begin_h <- DR_result_h[row_h, 'start']
		end_h <- DR_result_h[row_h, 'end']
...
for (Kgroup_h in Kgroups_uniq)
{
	...
	DR_result_h <- read.delim(file=DR_filename, header=TRUE, sep = "\t")
	DR_result_h <- DR_result_h[DR_result_h$score >= opt$score,]
	...
	n_seqs_r <- n_distinct(DR_result_h$seqID) # seq have repeat

	if (nrow(DR_result_h)!=0) # ignore K group if no repeat detect
	{
		mt_MSA <- get_mt_MSA(MSA_h, MSA_len, n_seqs)
		mt_rep_loci <- get_mt_rep_loci(DR_result_h, MSA_h, mt_MSA, MSA_len, n_seqs) 
