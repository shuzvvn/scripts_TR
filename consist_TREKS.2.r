...
# --TREKS_dir=~/project/TR02/TR02.22/aa_DetectRepeats/ \
...
option_list = list(
	...
	make_option(c("-r", "--TREKS_dir"), type="character", default="NA", help="Directory path for Kgroup.xls (Required)", metavar="dirname"),
	make_option(c("-a", "--MSA_dir"), type="character", default="NA", help="Directory path for Kgroup.MSA.rds (Required)", metavar="dirname"),
	make_option(c("-s", "--score"), type="double", default=0.6, help="score cutoff (Required)", metavar="number"),
	make_option(c("-o", "--out_tsv"), type="character", default="NA", help="output tsv (Required)", metavar="filename")
); 

# get command line options, if help option encountered print help and exit,
# otherwise if options not found on command line then set defaults,
opt <- parse_args(OptionParser(usage = "%prog [options] [--Kgroups_uniq=FILENAME] [--TREKS_dir=DIRNAME] [--MSA_dir=DIRNAME] [--score=FLOAT] [--out_tsv=FILENAME]", option_list=option_list)) 
...
get_mt_rep_loci <- function(TREKS_result_h, MSA_h, mt_MSA, MSA_len, n_seqs) {
    mt_rep_loci <- matrix(0, nrow = MSA_len, ncol = n_seqs)
    for (row_h in seq(nrow(TREKS_result_h)))
    {
        index_h <- which(startsWith(names(MSA_h), TREKS_result_h[row_h, 'seqID']))
        begin_h <- TREKS_result_h[row_h, 'start']
        end_h <- TREKS_result_h[row_h, 'end']
...
line <- paste('KgroupID', 'n_seqs', 'n_seqs_r', 'n_pairs', 'n_seqs_inpairs', 'mean_frac_repeat', 'mean_cons', sep='\t')
write(line, file=opt$out_tsv, append=FALSE)

for (Kgroup_h in Kgroups_uniq)
{
	# get the filtered results
	TREKS_filename <- paste(opt$TREKS_dir, Kgroup_h, '.tsv', sep='')
    TREKS_result_h <- read.delim(file=TREKS_filename, header=TRUE, sep = "\t")
    TREKS_result_h <- TREKS_result_h[TREKS_result_h$score >= opt$score,]
	...
	n_seqs_r <- n_distinct(TREKS_result_h$seqID) # seq have repeat

	if (nrow(TREKS_result_h)!=0) # ignore K group if no repeat detect
	{
        mt_MSA <- get_mt_MSA(MSA_h, MSA_len, n_seqs)
        mt_rep_loci <- get_mt_rep_loci(TREKS_result_h, MSA_h, mt_MSA, MSA_len, n_seqs) 
		...
		{
			#print(mt_rep_loci[,seqs_included])
            frac_repeat_h <- mean(get_frac_repeat_h(mt_rep_loci[,seqs_included]))
            mean_h <- mean(pair_consistency)
			line <- paste(Kgroup_h, n_seqs, n_seqs_r, length(pair_consistency), length(unique(seqs_included)), frac_repeat_h, mean_h, sep='\t')
			write(line, file=opt$out_tsv, append=TRUE)
		} else {
			line <- paste(Kgroup_h, n_seqs, n_seqs_r, 0, 0, 0, NA, sep='\t')
			write(line, file=opt$out_tsv, append=TRUE)
		}
	} else {
